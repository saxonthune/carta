...
Every guard failure across the action catalog surfaces as a named `ERR-*` code. The catalog is the contract between guards (in command sidecars), exception classes (in `rhidoc/errors.py`), and CLI exit messages.
