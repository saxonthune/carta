...
The full command catalog — guards, effects, invariants preserved, error modes — lives in doc03.01 (Rhidoc Docs API — Design). All structural operations maintain cross-reference integrity and treat **bundles** (root doc + same-prefix sidecars) as units.
...
Both invoke the same `commands.py` implementation. The portable form is the default for new workspaces — it embodies the right-to-repair principle (doc01.01).
