...
After running the `/rhidoc-feature-implementor` skill through several grooming sessions, we audited where tokens were spent versus where they provided value. The findings generalize to any multi-phase skill or background agent.
...
- Updated `/rhidoc-feature-implementor` SKILL.md with lean debrief approach (Phase 0)
