...
Architecture considerations for comparing `.rhidoc/` specifications against source code. This document will evolve as reconciliation moves from research (doc03.08.04) into implementation.
...
2. **Docs API is separate**: The Rhidoc Docs API (doc03.06.01) manages workspace structure. Reconciliation tooling is a distinct concern that may *use* the Docs API but is not part of it.
...
- Should reconciliation be a `rhidoc` subcommand, a separate tool, or an AI agent workflow?
- How do we handle the gap between deterministic extraction (what scripts can do) and semantic comparison (what LLMs can do)?

## References

- doc03.07 — Feature description (spec-code reconciliation)
- doc03.08.04 — Research session (two-source-of-truth model, data formats, script architecture)
