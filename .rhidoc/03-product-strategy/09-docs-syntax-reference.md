...
A lightweight specification for the `.rhidoc/` document syntax. Intended as a machine-readable reference for AI agents and tooling authors. Covers the grammar of document references, in-file sections, frontmatter schema, and extraction algorithms.
...
doc03.01        → 04-product-strategy/01-mission.md
doc03.08.03     → 04-product-strategy/08-research/03-decomposition-and-composition-theory.md
```
...
```
doc02.03#sec02      → file doc02.03, section 02
doc02.03#sec01.02   → file doc02.03, section 01, subsection 02
```
...
Every `.rhidoc/` document begins with YAML frontmatter.
...
deps: [doc03.01, doc02.01]            # optional: doc refs to check on change
...
- Orphan: an attachment whose prefix has no corresponding `.md` root, or whose prefix matches a directory. Triggers a warning from `rhidoc regenerate`; never blocks operation.

## #sec06 MANIFEST.md Structure

MANIFEST.md is the machine-readable retrieval index. It is generated by `rhidoc regenerate` and should not be edited by hand.
...
|-----|------|---------|------|------|------|
| doc03.03 | `02-glossary.md` | Canonical vocabulary | glossary, terms | doc03.01 | — |
```
...
3. **Subdocuments** — when a section outgrows its host, `rhidoc punch` expands the file into a directory. The section becomes its own doc with its own potential sections.
