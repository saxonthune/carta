...
This mirrors the `implements` / `depends-on` / `relates-to` reference system from the spec ladder (doc03.08.04). Each structure type is a node kind; references between them are typed edges. The full product model is a graph of these structures.
