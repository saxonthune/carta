...
**Value for Rhidoc:** Rhidoc's metamodel (schemas as objects, connections as morphisms, compilation as a functor to text) is genuinely categorical. The typed-port system and polarity model are monoidal structure. So CT is useful for *Rhidoc's internals*, even if it doesn't help with the *user's* decomposition problem.
