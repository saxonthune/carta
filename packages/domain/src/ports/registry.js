 ...
 */
export const DEFAULT_PORT_SCHEMAS = builtInPortSchemas;
/**
...
export class PortRegistry {
    schemas;
    constructor(schemas = DEFAULT_PORT_SCHEMAS) {
     ...
     */
    matchesCompatibility(rules, other) {
     ...
     */
    getEffectiveDirection(polarity) {
        if (polarity === 'relay')
            return 'source';
        if (polarity === 'intercept')
            return 'sink';
        return polarity;
    }
    /**
     ...
     */
    canConnect(fromPortType, toPortType) {
        ...
        if (fromDir === 'source' && toDir === 'source')
            return false;
        if (fromDir === 'sink' && toDir === 'sink')
            return false;
 ...
 */
export function syncWithDocumentStore(portSchemas) {
    if (portSchemas) {
