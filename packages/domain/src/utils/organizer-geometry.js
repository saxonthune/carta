 ...
 */
export const DEFAULT_ORGANIZER_LAYOUT = {
    padding: 20,
    headerHeight: 40,
};
/**
 ...
 */
export function computeOrganizerBounds(nodes, config = DEFAULT_ORGANIZER_LAYOUT) {
    if (nodes.length === 0) {
 ...
 */
export function toRelativePosition(nodePos, parentPos) {
    return {
 ...
 */
export function toAbsolutePosition(nodePos, parentPos) {
    return {
        x: nodePos.x + parentPos.x,
        y: nodePos.y + parentPos.y,
    };
 ...
 */
export function computeMinOrganizerSize(children, config = DEFAULT_ORGANIZER_LAYOUT) {
 ...
 * Children positions are assumed to be relative to the organizer.
 */
export function computeOrganizerFit(children, config = DEFAULT_ORGANIZER_LAYOUT) {
    const noShift = {
 ...
 */
export function nodeOverlapsOrganizer(nodePos, nodeSize, organizerPos, organizerSize) {
    ...
    return (nodePos.x < organizerRight &&
 ...
 */
export function nodeContainedInOrganizer(nodePos, nodeSize, organizerPos, organizerSize) {
    const nodeRight = nodePos.x + nodeSize.width;
    const nodeBottom = nodePos.y + nodeSize.height;
    const organizerRight = organizerPos.x + organizerSize.width;
    const organizerBottom = organizerPos.y + organizerSize.height;
    return (nodePos.x >= organizerPos.x &&
