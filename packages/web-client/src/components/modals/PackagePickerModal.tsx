...
function DocumentPackageCard({ item }: { item: DocumentPackageItem }) {
...
function DocumentTab({ packages, unpackagedSchemas, onCreatePackage }: DocumentTabProps) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {packages.map((item) => (
        <DocumentPackageCard key={item.package.id} item={item} />
      ))}
      {showCreateForm ? (
        <CreatePackageForm
