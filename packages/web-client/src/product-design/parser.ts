...
import type { CartaCodeBlock, EnumerationData, ParsedDocument } from './types.js';
...
function parseCartaBlock(
  ...
  if (separatorIndex === -1) {
    console.warn('[carta] Skipping block: missing --- separator');
    return null;
  }
  ...
  try {
    ...
    if (typeof frontmatter?.name !== 'string' || typeof frontmatter?.type !== 'string') {
      console.warn('[carta] Skipping block: frontmatter missing name or type', frontmatter);
      return null;
    }

    return { name: frontmatter.name, type: frontmatter.type, body: body ?? {} };
  } catch (err) {
    console.warn('[carta] Skipping block: YAML parse error', err);
    return null;
  }
}
...
export function updateBlock(
  ...
  // Reconstruct the original frontmatter by re-parsing the source block
...
/** Convenience: parse enumeration body into typed data */
export function asEnumeration(block: CartaCodeBlock): EnumerationData {
  const { body } = block;
  const kind = body.kind === 'ordinal' || body.kind === 'nominal' ? body.kind : 'nominal';
  const rawValues = Array.isArray(body.values) ? body.values : [];
  const values = rawValues
    .filter((v): v is Record<string, unknown> => v !== null && typeof v === 'object')
    .map((v) => ({
      key: String(v.key ?? ''),
      ...(v.remark !== undefined ? { remark: String(v.remark) } : {}),
    }));
  return { kind, values };
}
