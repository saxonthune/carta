...
function importIntoLevel(
  ...
  // Flatten all file levels' nodes, edges, and deployables
  const allNodes = data.levels.flatMap(l => l.nodes) as Node[];
  const allEdges = data.levels.flatMap(l => l.edges) as Edge[];
  const allDeployables = data.levels.flatMap(l => l.deployables);

  // Import deployables additively
  const levelDeployables = allDeployables.filter(d => config.deployables.has(d.id));
  if (levelDeployables.length > 0) {
    const existing = adapter.getDeployables();
    const existingIds = new Set(existing.map(d => d.id));
    const newDeployables = levelDeployables.filter(d => !existingIds.has(d.id));
    if (newDeployables.length > 0) {
      adapter.setDeployables([...existing, ...newDeployables]);
    }
  }
