...
import { listConstructs, getConstruct, createConstruct, updateConstruct, deleteConstruct, connect, disconnect, listSchemas, getSchema, createSchema, removeSchema, listDeployables, createDeployable, compile, extractDocument, } from '@carta/document';
// ===== CONSTANTS =====
 ...
 */
export function getActiveLevelId(ydoc) {
    const ymeta = ydoc.getMap('meta');
    const active = ymeta.get('activeLevel');
    if (active)
        return active;
    ...
    ylevels.forEach((ylevel, id) => {
        const order = ylevel.get('order') ?? 0;
        if (order < firstOrder) {
            firstOrder = order;
            firstId = id;
        }
 ...
 */
export function createDocumentServer(config) {
    ...
    function broadcastUpdate(docState, update, origin) {
        const encoder = encoding.createEncoder();
        encoding.writeVarUint(encoder, MESSAGE_SYNC);
    ...
    function handleMessage(conn, docState, message) {
        const { doc } = docState;
        try {
    ...
    async function setupWSConnection(conn, docName) {
        ...
        const updateHandler = (update, origin) => {
            if (origin !== conn && origin !== 'local') {
                broadcastUpdate(docState, update, origin);
            }
        };
        ...
        try {
            ...
            if (path === '/api/documents' && method === 'POST') {
                ...
                docState.doc.transact(() => {
                    ...
                    const ylevels = docState.doc.getMap('levels');
                    if (ylevels.size === 0) {
            ...
            if (constructsMatch) {
                ...
                if (method === 'POST') {
                    ...
                    const construct = createConstruct(docState.doc, getActiveLevelId(docState.doc), body.constructType, body.values || {}, { x: body.x || 100, y: body.y || 100 });
            ...
            if (connectionsMatch) {
                ...
                if (method === 'POST') {
                    ...
                    const edge = connect(docState.doc, getActiveLevelId(docState.doc), body.sourceSemanticId, body.sourcePortId, body.targetSemanticId, body.targetPortId);
                    if (!edge) {
                ...
                if (method === 'DELETE') {
                    ...
                    const disconnected = disconnect(docState.doc, getActiveLevelId(docState.doc), body.sourceSemanticId, body.sourcePortId, body.targetSemanticId);
            ...
            if (deployablesMatch) {
                ...
                if (method === 'POST') {
                    ...
                    const deployable = createDeployable(docState.doc, getActiveLevelId(docState.doc), body.name, body.description, body.color);
