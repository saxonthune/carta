...
deps: [doc01.08.09, doc01.06.04, doc01.04.01]
---
...
**Storage:** JSON, with markdown export for AI consumption. See doc01.06.04.
...
This mirrors the `implements` / `depends-on` / `relates-to` reference system from the spec ladder (doc01.08.06). Each structure type is a node kind; references between them are typed edges. The full product model is a graph of these structures.
