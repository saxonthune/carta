...
**Superseded by ADR 009** (filesystem-first workspace). The workspace model replaces server-managed documents with git-backed files. Non-developer access is handled by the workspace server's git integration (see doc01.03.03, Team Workspace) rather than a SaaS platform.
