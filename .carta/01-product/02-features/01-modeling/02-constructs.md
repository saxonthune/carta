...
Every construct is an instance of a construct schema (doc01.02.01.06). The schema defines:
...
Constructs can be grouped into organizers (doc02.09) for spatial organization on the canvas. Organizers are visual-only — they are never compiled. See doc01.01.03 for the distinction between organizers (spatial) and connections (semantic).
