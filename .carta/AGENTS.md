# Working in this `.carta/` workspace

This file is generated by `carta init` and refreshed by `carta init --rehydrate`.
Do not hand-edit — change it upstream in carta, or keep project-specific guidance
in your `CLAUDE.md` / `AGENTS.md` instead.

`.carta/` is a **Carta workspace**: numbered Markdown docs with stable
cross-references and an auto-generated `MANIFEST.md`. It is the spec/design source
of truth for this project.

## The four rules

1. **Read `.carta/MANIFEST.md` first.** It is the machine-readable index —
...
2. **Content edits use Write/Edit; structural changes use the `carta` CLI.**
   Editing prose inside an existing doc is a normal file edit — then run
   `carta regenerate`. Creating, moving, deleting, renaming, punching, or
   flattening docs must go through `carta`. Direct file writes bypass numbering,
...
4. **Run `carta ai-skill` before using any `carta` command.** It prints the
...
read its `SKILL.md` directly. The `carta` CLI itself works everywhere — `carta`
on PATH, or `python3 .carta/carta.py` if the workspace was made portable.
