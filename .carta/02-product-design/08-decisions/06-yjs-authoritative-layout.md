...
- **Stale reads**: `reactFlow.getNodes()` returns stale data immediately after `reactFlow.setNodes()` in the same synchronous block (doc01.08.03). Requires passing "known positions" forward as parameters — a fragile workaround.
