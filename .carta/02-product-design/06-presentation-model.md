...
The ctrl+drag handlers read fresh React Flow state before position conversion. This prevents bugs where stale hook state causes incorrect relative/absolute position calculations. The operations are in `useLayoutActions` because they follow the same 3-layer sync pattern as other layout operations (see doc03.06).
...
This feature was designed in research session doc01.08.01 to address visual clarity in BPMN-style sequential processes. The badges are presentation-layer only — the compiler never sees them.
...
The shape variants (`circle`, `diamond`, `document`, `stadium`, `parallelogram`) support notation-specific rendering for BPMN, ISO 5807 flowcharts, and other visual languages (see doc01.08.01). Circle renders as a circular node (events, states), diamond as a diamond shape (gateways, decisions), document as a document-shaped icon (artifacts, data objects), stadium as a capsule/pill with fully rounded ends (flowchart terminals), and parallelogram as a skewed quadrilateral (flowchart data/I/O).
...
The presentation model lives in the **Visual Editor Layer** (doc03.01) but is conceptually distinct from React components. It is the bridge between "what exists" and "what you see."
