...
- **Organizers**: Visual grouping containers with layout strategies (per-page). See doc01.04.08
- **Pages**: Separate architectural views, each with own nodes/edges
- **Spec Groups**: Document-level organizational groups containing ordered list of pages (doc01.03.09)
...
- `usePresentation()` — transforms domain state into view state (node visibility, positioning, edge remapping) via the presentation model (doc01.04.08)
