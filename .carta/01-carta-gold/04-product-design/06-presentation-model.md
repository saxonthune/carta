...
The dual mandate (doc01.05.01) requires Carta to be useful for both human visual organization and AI-readable output. Organizers serve the human; connections serve the AI. If these were conflated:
...
- Users would have two ways to express "A contains B" (put B in A's organizer, OR connect B's child port to A's parent port), violating "Necessary and Sufficient Primitives" (doc01.05.02)
...
The ctrl+drag handlers read fresh React Flow state before position conversion. This prevents bugs where stale hook state causes incorrect relative/absolute position calculations. The operations are in `useLayoutActions` because they follow the same 3-layer sync pattern as other layout operations (see doc01.03.06).
...
This feature was designed in research session doc01.05.08.01 to address visual clarity in BPMN-style sequential processes. The badges are presentation-layer only — the compiler never sees them.
...
The shape variants (`circle`, `diamond`, `document`, `stadium`, `parallelogram`) support notation-specific rendering for BPMN, ISO 5807 flowcharts, and other visual languages (see doc01.05.08.01). Circle renders as a circular node (events, states), diamond as a diamond shape (gateways, decisions), document as a document-shaped icon (artifacts, data objects), stadium as a capsule/pill with fully rounded ends (flowchart terminals), and parallelogram as a skewed quadrilateral (flowchart data/I/O).
...
The presentation model lives in the **Visual Editor Layer** (doc01.03.01) but is conceptually distinct from React components. It is the bridge between "what exists" and "what you see."
