...
Canvas-specific vocabulary. For workspace/spec vocabulary, see doc01.05.08.
...
**Organizer**: A canvas-level grouping mechanism for visual organization. Never compiled. Constructs inside an organizer are **members**, not children — "parent/child" is reserved for the port system. See doc01.04.08.

**Layout Strategy**: The arrangement algorithm used by an organizer. Currently only `freeform` is implemented.

**Presentation Model**: A stateless transformation layer that converts domain state into view state. See doc01.04.08.
