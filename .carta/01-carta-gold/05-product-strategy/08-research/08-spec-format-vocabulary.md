...
This is not just about *arrangement* (how specs are grouped into directories). It's a whole set of topics: arrangement, typing, granularity, abstraction level, prose style, dependency topology, and more. The principle (doc01.05.02, "Spec Agnosticism") is that the format provides structural machinery and stays out of everything else.
...
All arrangement decisions come from the user or their AI agent after init. In the web platform's conversational flow (doc01.03.04), the agent can scaffold the workspace interactively — "What's this project about? Let me set up a workspace for you." This keeps arrangement decisions in the user's session rather than baked into the CLI.
