...
deps: [doc01.05.08.10, doc01.03.07, doc01.05.06.02]
---

# Product Design UI

Rework the canvas engine to support visual editors for all structured product modeling types (doc01.05.08.10). The goal is tools that product owners and domain experts are happy to use — not developer tools with a friendlier skin.

## Scope

The canvas engine (doc01.03.07) is already domain-agnostic. This project extends it to support the full range of product design structures: entity models, ER diagrams, decision tables, state machines, flowcharts, enumerations, and more. Each structure type gets a visual editor built on shared canvas primitives.
...
- **Naming**: the editor types are called "structures" (not metaphors, instruments, surfaces). Matches doc01.05.08.10 vocabulary.
...
- **Project as staging ground**: insights developed here land in doc01.05–04 when ready. The project tracks the work, not the canonical product descriptions.
...
- **Delivery via VS Code extension**: the product design canvas is delivered as a VS Code custom editor. User opens a canvas file → extension reads source markdown files from the filesystem → posts parsed data to a WebView via `postMessage` → WebView renders the `ProductDesignCanvas` component. Edits flow back: WebView posts changes → extension writes to filesystem. The extension handles file watching (`fs.watch`) and re-posts updated data when source files change externally (e.g., AI edits via script API). No server, no WebSocket, no Yjs — the extension IS the file bridge. This is the same pattern as the legacy canvas (`CartaCanvasEditorProvider`) but with direct filesystem I/O replacing Yjs sync.
- **No server for product design canvas**: unlike the legacy canvas which uses an embedded server + Yjs rooms + WebSocket sync, the product design canvas operates entirely through the VS Code extension's filesystem access. The server is not involved. This matches the doc01.01.01.04 data flow: canvas and AI both interact with the filesystem directly, not through each other.
- **Editor component API**: all structure editors use the controlled component pattern (`value`/`onChange`). The canvas is also controlled (`files`/`layout` in, `onBlockChange`/`onLayoutChange` out). This makes the WebView ↔ extension boundary clean: the extension serializes data in, the WebView serializes callbacks out.
...
| Ref | Item | Summary |
|-----|------|---------|
| doc01.01.01.01 | Gap Analysis | What exists today vs what's needed |
| doc01.01.01.02 | Engine Changes | Canvas engine rework required to support the editor types |
| doc01.01.01.03 | Structures | The product design structures and their editors (section) |
| doc01.01.01.03.01 | Enumerations | Data model, YAML format, UI design, interaction vocabulary |
| doc01.01.01.03.02 | Process Flow | Flowchart structure — data model, canvas interactions, engine requirements |
| doc01.01.01.04 | User Experience | How users interact with structures — nouns, verbs, flows |
