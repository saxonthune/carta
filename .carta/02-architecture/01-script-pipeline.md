...
Architecture considerations for comparing `.carta/` specifications against source code. This document will evolve as reconciliation moves from research (doc04.08.04) into implementation.
...
2. **Docs API is separate**: The Carta Docs API (doc04.06.01) manages workspace structure. Reconciliation tooling is a distinct concern that may *use* the Docs API but is not part of it.
...
- doc04.07 — Feature description (spec-code reconciliation)
- doc04.08.04 — Research session (two-source-of-truth model, data formats, script architecture)
