...
- `<destination>` — ref or path to an existing directory (e.g. `doc01.05`, `01-product`)
...
carta create doc01.05 new-feature --order 3           # create at position 03
carta create doc01.05.08 my-section --title "My Section" # custom title
...
- Accepts refs (`doc01.05.02`) or paths (`01-product/02-features`)
...
**Examples**:
```bash
carta delete doc01.05.08.01                                # delete a single doc
carta delete doc01.05.02 doc01.05.08                          # delete multiple entries
carta delete doc01.05.08.01 --dry-run                      # preview deletions + orphan warnings
carta delete doc01.05.08.01 --output-mapping > map.json    # capture rename map for rewrite
```
...
**Examples**:
```bash
carta move doc01.05.05 doc01.05 --order 2                           # promote into parent
carta move doc01.05.05 . --rename diagramming                   # rename slug in place
carta move doc01.05.05 doc01.05 --order 2 --rename diagramming     # move + rename
carta move doc01.05.08 01-product --mkdir --order 3 --rename research  # move into new dir
```
...
**Example**:
```bash
carta punch doc01.03.04    # 01-canvas.md → 01-canvas/00-index.md
```
...
**Examples**:
```bash
carta flatten doc01.05.02                # dissolve features/, promote children
carta flatten doc01.05.02 --keep-index   # same, but keep index as sibling
```
...
carta delete doc01.05.02 --output-mapping > /tmp/delete-map.json

# ... rebuild structure ...

# Restore a file from backup
carta copy /tmp/backup/03-my-doc.md doc01.05.05 --rename my-doc
