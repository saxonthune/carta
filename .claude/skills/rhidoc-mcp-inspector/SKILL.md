---
name: rhidoc-mcp-inspector
description: Audits Rhidoc's MCP tools against best practices and generates improvement recommendations with implementation guidance
---

# rhidoc-mcp-inspector

Audits Rhidoc's MCP tool surface against established best practices and generates actionable improvement recommendations. Can also be pointed at the current conversation to diagnose tool-use friction in real time.

## When to invoke

- After experiencing friction using Rhidoc MCP tools (too many round trips, missing operations, bad errors)
...
**Pattern**: `rhidoc_batch_mutate` accepting an array of heterogeneous operations (create, update, delete, connect, move) in one transaction.
...
- `rhidoc_move_construct` — change parentId (organizer membership) without delete/recreate
- `rhidoc_auto_layout` — reposition constructs to avoid overlaps
- `rhidoc_reorganize` — move constructs into organizers while preserving connections
- `rhidoc_clone_construct` — deep copy with new semanticId
...
**Pattern**: `{ "error": "Construct 'abc-123' not found on the active level. It may exist on a different level — try rhidoc_list_constructs with levelId to locate it.", "suggestion": "rhidoc_list_levels" }`
...
1. **rhidoc_move_construct** — Severity: high
