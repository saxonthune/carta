...
# --- Determine diff base ---
DOCS_BASE=""
if [ -f .rhidoc/.last-sync ]; then
  CANDIDATE=$(cat .rhidoc/.last-sync | tr -d '[:space:]')
  if git merge-base --is-ancestor "$CANDIDATE" HEAD 2>/dev/null; then
    DOCS_BASE="$CANDIDATE"
  fi
fi
if [ -z "$DOCS_BASE" ]; then
  DOCS_BASE=$(git log -1 --format=%H -- .rhidoc/)
fi
if [ -z "$DOCS_BASE" ]; then
  DOCS_BASE="HEAD~20"
fi
...
# --- Changed files by package (excluding .rhidoc/) ---
echo "### Changed Files by Package"
echo ""
git diff --stat "$DOCS_BASE"..HEAD -- ':!.rhidoc/' ':!.claude/' ':!pnpm-lock.yaml' | tail -1
echo ""
git diff --name-only "$DOCS_BASE"..HEAD -- ':!.rhidoc/' ':!.claude/' ':!pnpm-lock.yaml' | \
...
echo "### New Files"
echo ""
git diff --diff-filter=A --name-only "$DOCS_BASE"..HEAD -- ':!.rhidoc/' ':!.claude/' ':!pnpm-lock.yaml' | head -30
echo ""

echo "### Deleted Files"
echo ""
git diff --diff-filter=D --name-only "$DOCS_BASE"..HEAD -- ':!.rhidoc/' ':!.claude/' ':!pnpm-lock.yaml' | head -30
echo ""

# --- What docs WERE touched (for comparison) ---
echo "### Docs Touched in Range"
echo ""
DOCS_CHANGED=$(git diff --name-only "$DOCS_BASE"..HEAD -- '.rhidoc/' | head -20)
