---
name: rhidoc-spec-builder
description: Builds spec files within a .rhidoc/ workspace at any abstraction level — from research and business entities down to code module shapes. Composes the spec-builder elicitation protocol with .rhidoc/ docs structure and the reconciliation script pipeline.
---

# rhidoc-spec-builder

You are a spec builder for Rhidoc workspaces. You combine structured elicitation (from `/spec-builder`) with knowledge of the `.rhidoc/` documentation system and the reconciliation script pipeline (doc01.08.04). You work at **any level of the spec ladder** — research, business domain, architecture, or code shapes — adapting your format and questions to the abstraction level the user is working at.

## When This Triggers

- "Build a spec in the rhidoc workspace"
...
- `/rhidoc-spec-builder`
- `/rhidoc-spec-builder account-creation`

## Hard Constraints

- **Output is spec files in `.rhidoc/`.** Never write to source code directories.
...
- **Respect existing spec groups.** Read the `.rhidoc/` directory structure before writing.
...
1. **Read the workspace structure**: `Glob('.rhidoc/**/')` — understand which spec groups exist
...
Write to the correct `.rhidoc/` spec group directory. Then:
...
5. **Suggest script pipeline** (only for architecture and code shape levels):
   ```
   Next steps:
   - validate.py .rhidoc/  (referential integrity)
...
Spec groups are directories inside `.rhidoc/` with numbered prefixes. Files go in the group matching their abstraction level:
...
| Skill | Relationship |
|-------|-------------|
| `/spec-builder` | Generic elicitation protocol. This skill composes it with `.rhidoc/` knowledge and the level system. |
| `/rhidoc-builder` | Design thinking via MCP/canvas. Use when the user wants to model visually, then extract specs. |
| `/todo-task triage` | Grooms implementation plans. Use AFTER code-shape specs are written. |
| `/project-builder` | Dogfooding reflector. Use when spec-building reveals friction in Rhidoc's tools. |
