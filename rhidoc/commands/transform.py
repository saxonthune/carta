...
def cmd_punch(args: argparse.Namespace, rhidoc_root: Path) -> None:
    ...
    if not source_path.name.endswith(".md"):
        raise RhidocError(f"Error: source is not a .md file: {source_path}")
...
def cmd_flatten(args: argparse.Namespace, rhidoc_root: Path) -> None:
    ...
    if has_index and not keep_index:
        ...
        if content_lines > 10 and not force:
            raise RhidocError(
                f"Error: {index_file.relative_to(rhidoc_root)} has {content_lines} "
    ...
    if not hoisted:
        raise RhidocError("Error: no children to hoist.")
...
def cmd_copy(args: argparse.Namespace, rhidoc_root: Path) -> None:
    ...
    if args.at is not None or args.before is not None:
        ...
        if strict:
            ...
            if target_prefix in occupied:
                raise RhidocError(
                    f"Error: position {target_prefix:02d} is occupied in {dest_path.relative_to(rhidoc_root)}.\n"
        ...
        if prefix in occupied:
            raise RhidocError(
                f"Error: position {prefix:02d} is occupied in {dest_path.relative_to(rhidoc_root)}.\n"
                f"Occupied positions: {sorted(occupied)}"
            )
    ...
    print(f"Copied: {source_path.name} -> {new_path.relative_to(rhidoc_root)}")
    print(f"  Position: {prefix:02d}")
    if shift_moves:
