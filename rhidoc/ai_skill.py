...
def generate_skill_content(dir_name: str) -> str:
    ...
    sections = [render("rhidoc-cli", dir_name=dir_name).rstrip()]
...
def _workspace_state_section(rhidoc_root: Path) -> list[str]:
    ...
    lines.append(f"Workspace root: `{rhidoc_root.name}/`")
    lines.append("")

    top_entries = list_numbered_entries(rhidoc_root)
    total_docs = 0
    total_sidecars = 0
...
def cmd_ai_skill(args: argparse.Namespace, rhidoc_root: Path) -> None:
    ...
    lines.append("")
    lines.extend(_workspace_state_section(rhidoc_root))
