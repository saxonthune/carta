"""DocRef and EntryName — the two formal grammars for rhidoc doc references.
...
class DocRef:
    """Coordinate for a .rhidoc/ document: XX.YY.ZZ as a tuple of ints.
    ...
    def parse(cls, raw: str) -> DocRef:
        ...
        Raises RhidocError on malformed input.
        """
        s = raw.strip()
        ...
        if not s:
            raise RhidocError(f"Invalid doc ref: {raw!r}")
    ...
    def from_path(cls, path: Path, rhidoc_root: Path) -> DocRef:
        ...
        path component relative to rhidoc_root.
...
@dataclass(frozen=True)
class EntryName:
    """Decomposition of a .rhidoc/ filesystem entry name into its structural parts.
