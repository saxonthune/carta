...
This file is generated by `rhidoc init` and refreshed by `rhidoc update`.
Do not hand-edit — change it upstream in rhidoc, or keep project-specific guidance
in your `CLAUDE.md` / `AGENTS.md` instead.

`{{dir_name}}/` is a **Rhidoc workspace**: numbered Markdown docs with stable
...
`doc00.00` through `doc00.06` are rhidoc's, not yours. `rhidoc update` overwrites them
from the installed version, so an edit there is lost on the next update. To change one,
change it upstream in rhidoc.

`doc00.07` (`07-user-handbook/`) is yours. Rhidoc creates it empty and never touches it
again — put your project's own doctrine there. Rhidoc's own handbook docs continue at
`08`, so the slot stays free.
...
on PATH, or `python3 {{dir_name}}/rhidoc.py` if the workspace was made portable.
